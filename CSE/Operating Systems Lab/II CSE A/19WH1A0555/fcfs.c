#include<stdio.h>
void get_waiting_time(int processor[],int n,int bt[],int wt[]){
    wt[0] = 0;
    for(int i = 1;i < n;i ++){
        wt[i] = bt[i - 1] + wt[i -1];
    }
}
void get_turnaround_time(int processor[],int n,int bt[],int wt[],int tat[]){
    for(int i = 0;i < n;i ++){
        tat[i] = bt[i] + wt[i];
    }
}
...
int main(){
    ...
    for(int i = 0;i < num; i ++){
        printf("Enter the burst time of the processes %d:",processes[i]);
